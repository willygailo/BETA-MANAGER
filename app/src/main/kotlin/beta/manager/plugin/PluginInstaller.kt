...
class PluginInstaller(
    ...
    fun interface ProgressCallback {
        fun onProgress(step: Int, total: Int, label: String)
    }

    fun installSync(zipPath: String): Boolean {
        val result = installInternalSync(zipPath, flashToMagisk = false)
        return result.success
    }

    fun installToMagiskSync(zipPath: String): InstallResult {
        ...
        return installInternalSync(zipPath, flashToMagisk = true, magiskTarget = targetDir)
    }

    fun installToKSUSync(zipPath: String): InstallResult {
    ...
    fun checkForUpdatesSync(plugins: List<PluginInfo>): List<UpdateInfo> {
    ...
    private fun installInternalSync(
        zipPath: String,
        ...
        try {
            ...
            val chmodResult = Shell.executeSyncWithElevation("chmod -R 755 ${Shell.quote(targetDir)}")
            ...
            if (customizeSh.exists()) {
                ...
                val customizeResult = Shell.executeSyncWithElevation(
            ...
            Shell.executeSyncWithElevation("rm -f ${Shell.quote("$targetDir/disable")} 2>/dev/null")
    ...
    suspend fun install(zipPath: String, progressCallback: ProgressCallback? = null): Boolean {
        val result = installInternal(zipPath, flashToMagisk = false, progressCallback = progressCallback)
        return result.success
    }

    suspend fun installToMagisk(zipPath: String, progressCallback: ProgressCallback? = null): InstallResult {
        val magiskModules = listOf(
            "/data/adb/modules/",
            "/data/adb/modules_update/"
        )
        ...
        val result = installInternal(zipPath, flashToMagisk = true, magiskTarget = targetDir, progressCallback = progressCallback)
        return result
    }

    suspend fun installToKSU(zipPath: String, progressCallback: ProgressCallback? = null): InstallResult {
        val ksuModules = listOf(
            "/data/adb/ksu/modules/",
            "/data/adb/modules/"
        )
    ...
    suspend fun installToAPatch(zipPath: String, progressCallback: ProgressCallback? = null): InstallResult {
        ...
        return installInternal(zipPath, flashToMagisk = true, magiskTarget = targetDir, progressCallback = progressCallback)
    }

    suspend fun installToAxeron(zipPath: String, progressCallback: ProgressCallback? = null): InstallResult {
    ...
    private suspend fun installInternal(
        ...
        progressCallback?.onProgress(1, totalSteps, "Validating")
        ...
        try {
            progressCallback?.onProgress(2, totalSteps, "Extracting")
            extractZip(zipFile, tempDir)
            ...
            progressCallback?.onProgress(3, totalSteps, "Cleaning")
            val removeResult = Shell.executeWithElevation("rm -rf ${Shell.quote(targetDir)}")
            if (removeResult is Shell.Result.Error) {
                return InstallResult(false, "Unable to clean target directory: ${removeResult.message}")
            }

            progressCallback?.onProgress(4, totalSteps, "Directories")
            val mkdirResult = Shell.executeWithElevation("mkdir -p ${Shell.quote(targetDir)}")
            if (mkdirResult is Shell.Result.Error) {
                return InstallResult(false, "Unable to create target directory: ${mkdirResult.message}")
            }

            progressCallback?.onProgress(5, totalSteps, "Copying")
            ...
            progressCallback?.onProgress(6, totalSteps, "Permissions")
            ...
            val customizeSh = File(tempDir, "customize.sh")
            if (customizeSh.exists()) {
                progressCallback?.onProgress(7, totalSteps, "customize.sh")
                val installedCustomizeSh = File(targetDir, "customize.sh").absolutePath
                val env: Map<String, String> = mapOf(
                    "BETA" to "true",
                    "BETAVER" to "${PluginManager.SERVER_VERSION}",
            ...
            progressCallback?.onProgress(8, totalSteps, "Finalizing")
            Shell.executeWithElevation("rm -f ${Shell.quote("$targetDir/disable")} 2>/dev/null")

            progressCallback?.onProgress(9, totalSteps, "Complete")
            val action = if (flashToMagisk) "Flashed to ${File(targetDir).parentFile?.name}" else "Installed"
            return InstallResult(true, "$action successfully: ${props["name"] ?: pluginId} (v${props["version"] ?: "1.0"})", pluginId)
